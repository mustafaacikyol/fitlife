                ...
                <div class="content col-4 text-center" style="margin-left: 350px;">
                    <?php
                        if(isset($_POST["shareBtn"])){
                            $generate = $conn->prepare("insert into client_nutrition set client_id=:client_id, nutrition_id=:nutrition_id");
                            $generate_result = $generate->execute(array("client_id" => $_POST["id"], "nutrition_id" => $_GET["id"]));
                            if (isset($generate_result)) {
                                echo "<div class='alert alert-icon alert-success' role='alert'>
                                    <em class='icon ni ni-check-circle'></em> 
                                    <strong>Nutrition shared successfully</strong> 
                                    </div>";
                                ...
                                <strong>Failed to Share Nutrition!</strong>
                                </div>";
                            }
                        }
